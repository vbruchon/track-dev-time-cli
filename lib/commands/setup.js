...
const installConcurrentlyWithConsent = () => {
  return new Promise((resolve, reject) => {
    ...
    rl.question(
      "Do you want to install 'concurrently' package now? (y/n): ",
      (answer) => {
        rl.close();
        ...
        if (normalized === "y" || normalized === "yes") {
          ...
          exec("npm install concurrently", (error, stdout, stderr) => {
            if (error) {
          ...
          console.log("Skipped installation of concurrently.");
          resolve(false);
        }
      }
    );
